...
/**
 * Creates cdoc2-auth-server session tokens (SD-JWT) for testing "x-cdoc2-session-token", so
 * that these tests don't depend on a running cdoc2-auth-server instance.
 * <p>
 * The token is signed with a fixed EC key ({@link TestDataGenerator#SESSION_TOKEN_SIGNING_KEY}),
 * whose public part must be served by whatever cdoc2-rp-server under test uses as its
...
public final class SessionTokenSigner {
    ...
    /**
     * Creates a session token for the given subject. The subject's matching certificate must be
     * passed separately as the "x-cdoc2-session-x5c" header, since the server cross-checks the
     * token subject against that certificate's identity.
     */
